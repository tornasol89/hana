  ...
  return (
    <div style={{ backgroundColor: '#1a0a10', minHeight: '100vh', color: 'white', fontFamily: 'sans-serif' }}>
      ...
      <section style={{ position: 'relative', height: isMobile ? '420px' : '560px', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(26,10,16,0.55) 0%, rgba(26,10,16,0.75) 100%)' }} />
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: isMobile ? '20px' : '48px', textAlign: 'center' }}>
          <div style={{ display: 'inline-block', background: 'rgba(212,83,126,0.25)', border: '1px solid #d4537e', color: '#ffb8d1', fontSize: '12px', padding: '6px 16px', borderRadius: '50px', marginBottom: '20px', letterSpacing: '1px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', marginTop: '28px' }}>
            {slides.map((_, i) => (
              <div key={i} onClick={() => setSlideActual(i)} style={{ width: i === slideActual ? '24px' : '8px', height: '8px', borderRadius: '50px', backgroundColor: i === slideActual ? '#d4537e' : 'rgba(255,255,255,0.4)', cursor: 'pointer', transition: 'all 0.3s ease' }} />
            ))}
          </div>
        </div>
      ...
      {/* BANNER */}
      <section style={{ background: 'linear-gradient(135deg, #d4537e 0%, #b83060 40%, #e8b86d 100%)', padding: isMobile ? '32px 20px' : '40px 48px', textAlign: 'center' }}>
      ...
      <section style={{ padding: isMobile ? '40px 20px' : '56px 48px', backgroundColor: '#1a0a10' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: '14px', maxWidth: '860px', margin: '0 auto' }}>
          {categorias.map((cat, i) => (
            <div key={cat.nombre} style={{ background: i >= 8 ? 'rgba(212,83,126,0.1)' : '#2d0a1e', border: i >= 8 ? '1px solid #d4537e' : '1px solid rgba(212,83,126,0.3)', borderRadius: '12px', padding: '20px 14px', textAlign: 'center', cursor: 'pointer' }}>
